...
## redis单机安装
1.官网下载redis包解压
2.yum install gcc-c++   ||  apt install gcc
3.make
4.make install
...
```
 1.注释 bind && 设置protected-mode no
 2.使用bind 0.0.0.0 && 没有设置密码
 3.设置密码
...
getrange  key  0(开始位置)  -1（结束位置）    获取字符串指定区间范围内的子串
...
在指定的时间间隔内，将内存当中的数据集快照写入磁盘，它恢复时是将快照文件直接读到内存
